...
from .litemono_head import LiteMonoCRF
from .uper_crf_head import PSP


class LiteMonoCRFDepth(nn.Module):
    def __init__(self, maxdisp, use_swin_backbone=True,
                 ...
                 disp_head='classify'):
        super(LiteMonoCRFDepth, self).__init__()
        ...
        win = 7
        lm_dims = [128, 256, 512, 1024]
        v_dims = [64, 128, 256, 512+6]
        self.lm3 = LiteMonoCRF(
            input_dim=in_channels[3],     embed_dim=lm_dims[3],  window_size=win, v_dim=v_dims[3])
        self.lm2 = LiteMonoCRF(
            input_dim=in_channels[2]+12,  embed_dim=lm_dims[2],  window_size=win, v_dim=v_dims[2])
        self.lm1 = LiteMonoCRF(
            input_dim=in_channels[1]+24,  embed_dim=lm_dims[1],  window_size=win, v_dim=v_dims[1])
        self.lm0 = LiteMonoCRF(
            input_dim=in_channels[0]+48,  embed_dim=lm_dims[0],  window_size=win, v_dim=v_dims[0])

        self.decoder = PSP(**decoder_cfg)
        self.disp_head1 = DispHead(
            input_dim=lm_dims[0], mode=disp_head, max_disp=self.maxdisp)
        self.disp_head2 = DispHead(
            input_dim=lm_dims[1], mode=disp_head, max_disp=self.maxdisp//2)
        self.disp_head3 = DispHead(
            input_dim=lm_dims[2], mode=disp_head, max_disp=self.maxdisp//4)
        self.disp_head4 = DispHead(
            input_dim=lm_dims[3], mode=disp_head, max_disp=self.maxdisp//4)
    ...
    def forward(self, left, right):
        ...
        e3 = self.lm3(feats_left[3], agg_volume_stereo)
        if self.training:
        ...
        e3 = self.lm3(feats_left[3], agg_volume_mono)
        if self.training:
            d3 = self.disp_head4(e3, self.scale_factor[3])
