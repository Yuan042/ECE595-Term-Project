...
class DilatedConv(nn.Module):
    ...
    def __init__(self, dim, k, dilation=1, stride=1, drop_path=0.,
        ...
        self.ddwconv = CDilated(
            dim, dim, kSize=k, stride=stride, groups=dim, d=dilation)
...
class LGFI(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.pos_embd:
            pos_encoding = self.pos_embd(B, H, W).reshape(
    ...
    def __init__(self, in_chans=3, model='lite-mono', height=192, width=640,
        ...
        if model == 'lite-mono':
            ...
            if height == 192 and width == 640:
                self.dilation = [[1, 2, 3], [1, 2, 3],
                                 [1, 2, 3, 1, 2, 3, 2, 4, 6]]
            elif height == 320 and width == 1024:
                self.dilation = [[1, 2, 5], [1, 2, 5],
                                 [1, 2, 5, 1, 2, 5, 2, 4, 10]]
        ...
        # --- normalize to 4 stages (H/4, H/8, H/16, H/32) ---

        # 1) backbone channel/深度/膨脹；不足就補到 4 階
        if len(self.dims) == 3:
            self.dims.append(self.dims[-1])          # C4 = C3
        if len(self.depth) == 3:
            self.depth.append(2)                     # 預設第 4 階放 2 個 blocks（可再調）
        if len(self.dilation) == 3:
            self.dilation.append([1, 2])             # 第 4 階的 dilation（可再調）
        if len(self.num_ch_enc) == 3:
            self.num_ch_enc = np.append(self.num_ch_enc, self.dims[-1])

        # 2) 其他超參（global_block / type / heads / 位置編碼）也補齊到 4
        #    傳進來的是函式參數（list），這裡轉成 4 長度的本地副本避免越界
        if isinstance(global_block, tuple):
            global_block = list(global_block)
        if isinstance(global_block_type, tuple):
            global_block_type = list(global_block_type)
        if isinstance(heads, tuple):
            heads = list(heads)
        if isinstance(use_pos_embd_xca, tuple):
            use_pos_embd_xca = list(use_pos_embd_xca)

        while len(global_block) < 4:
            global_block.append(global_block[-1])
        while len(global_block_type) < 4:
            global_block_type.append(global_block_type[-1])
        while len(heads) < 4:
            heads.append(heads[-1])
        while len(use_pos_embd_xca) < 4:
            use_pos_embd_xca.append(use_pos_embd_xca[-1])

        for g in global_block_type:
            assert g in ['None', 'LGFI'], f"unsupported global_block_type: {g}"

        # stem and 3 intermediate downsampling conv layers
        self.downsample_layers = nn.ModuleList()
        stem1 = nn.Sequential(
            Conv(in_chans, self.dims[0], kSize=3,
                 stride=2, padding=1, bn_act=True),
            Conv(self.dims[0], self.dims[0], kSize=3,
                 stride=1, padding=1, bn_act=True),
            Conv(self.dims[0], self.dims[0], kSize=3,
                 stride=1, padding=1, bn_act=True),
        )

        self.stem2 = nn.Sequential(
            Conv(self.dims[0]+3, self.dims[0], kSize=3,
                 stride=2, padding=1, bn_act=False),
        )
        ...
        for i in range(3):
            downsample_layer = nn.Sequential(
                Conv(self.dims[i]*2+3, self.dims[i+1], kSize=3,
                     stride=2, padding=1, bn_act=False),
            )
        ...
        dp_rates = [x.item() for x in torch.linspace(
            0, drop_path_rate, sum(self.depth))]
        cur = 0
        for i in range(4):
            stage_blocks = []
